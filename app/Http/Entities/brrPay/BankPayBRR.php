...
use App\Http\Entities\IBRRPayment;
use App\Http\Entities\BankBRRAuth;


class BankPayBRR implements IBRRPayment{
    ...
    public function subtractBRR(Request $request){
        ...
        $bankAuth = new BankBRRAuth();
        $balance = $bankAuth->getBalanceBRR($request);
        if($balance == -1){
